...
import ua.nure.notesapp.services.NotesService;
...
public class NotesListViewActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    Menu optionsMenu;
    String locale = "en";
    Importance _importanceFilter = Importance.ALL;
    String _searchText = "";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        _service = new NotesService(this);

        adapter = new NotesListViewAdapter(this, R.layout.note, _service.getAllNotes());
        ...
        switch (requestCode) {
            case REQUEST_CODE_1:
                if (resultCode == RESULT_OK) {
                    ...
                    if (savedNote != null) {
                        _service.addOrUpdate(savedNote);
                        refreshNotesList();
                    }
                }
        }
    ...
    private void setImportanceSpinner() {
        ...
        int initialPosition;
        if (_importanceFilter == Importance.ALL) {
            initialPosition = 0;
        } else {
            initialPosition = _importanceFilter.getValue();
        }
        ...
        spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
                if (pos == 0) {
                    _importanceFilter = Importance.ALL;
                } else if (pos == 1) {
    ...
    private void refreshNotesList() {
        ArrayList<Note> notes = _service.FilterAndSearch(_importanceFilter, _searchText);
        refreshListView(notes);
    }

    private void refreshListView(ArrayList<Note> notes) {
        adapter.setNewNoteList(notes);
